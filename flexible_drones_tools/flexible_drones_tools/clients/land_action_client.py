...
"""CLI client for the ``land`` action."""
...
USAGE = """\
Usage:
  ros2 run flexible_drones_tools land --ros-args -r __ns:=/drone1 -p height:=0.04 -p duration_sec:=2.0

Common examples:
  ros2 run flexible_drones_tools land --ros-args -p height:=0.04
  ros2 run flexible_drones_tools land --ros-args -r __ns:=/drone1 -p duration_sec:=3.0
  ros2 run flexible_drones_tools land --ros-args -r __ns:=/drone1 -r __node:=land_client -p group_mask:=0
...
Parameters declared by this node:
  group_mask (integer, default: 0)
    Drone group mask sent in the Land goal.
  height (double, default: 0.04)
    Target landing height in metres.
...
Examples with parameter overrides:
  ros2 run flexible_drones_tools land --ros-args -p duration_sec:=4.0
  ros2 run flexible_drones_tools land --ros-args -r __ns:=/drone2 -p group_mask:=1 -p height:=0.04
"""


class LandActionClient(Node):

    def __init__(self):
        super().__init__('land_action_client')
        ...
        self._action_client = ActionClient(
            self,
            Land,
            'land')  # Figure out how to add the name of each Drone here? (Anwser: Remapping the namespace via CLA!)
    ...
    def send_goal(self):
        ...
        goal_msg = Land.Goal()  # Assigns the variables recieved from the Action Server to Land.action's 'goal' field
        self.get_logger().info('LandActionClient accessed!')
        goal_msg.group_mask = group_mask
        goal_msg.height = height
        goal_msg.duration = Duration(seconds=duration_sec).to_msg()

        if not wait_for_action_server(self, self._action_client, 'Land action server'):
            return False

        self.get_logger().info('Sending land goal...')
    ...
    def goal_response_callback(self, future):
        ...
        if not goal_handle.accepted:
            self.get_logger().warning('Land goal rejected.')
            rclpy.shutdown()
            return  # If land goal is rejected, return early
    ...
    def get_result_callback(self, future):
        ...
        if result.return_code == 0:
            self.get_logger().info('Land finished successfully!')
        else:
            self.get_logger().warning(f'Land finished, but failed. return_code={result.return_code}')
...
def main(args=None):
    ...
    action_client = LandActionClient()
